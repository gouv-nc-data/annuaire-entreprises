    ...
    return (
        <div>
            <ul className='flex flex-col'>
                {
                    etablissements.slice(0, 3).map(etablissement =>
                        <li className="relative flex items-center border-s-1 border-slate-300">
                ...
                <Link to={`/`} className="relative before:w-6 before:h-4 before:border-s-1 before:absolute before:-top-3 before:left-0 before:border-slate-300">
                    ...
                    <span className='text-blue-800 font-normal text-md ps-6 hover:underline'>{`${etablissements.length} ${etablissements.length > 1 ? 'établissements' : 'établissement'} en activité`}</span>
                </Link>
            </ul>
        </div>
    )
