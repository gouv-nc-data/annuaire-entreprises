                        ...
                        <div className="flex items-center gap-2 p-4 pb-2 bg-white">
                            <div className="inline-flex items-center gap-2">
                                <div className="flex items-center gap-1 text-primary font-base text-slate-900 text-xs px-1 pe-[7px] !py-1 !h-auto border-1 border-zinc-300 shadow bg-white hover:bg-zinc-100 rounded-md">
                                    <History strokeWidth={1.2} className="w-4 h-4 text-slate-700 fill-blue-50" />
                                    {history.length}
                                </div>
                                <p className="text-slate-900 text-xs font-medium">Vos dernières recherche effectuées</p>
                            </div>
                        </div>
                        <ul className="flex flex-col bg-white ps-2 pb-2">
                            {
                                history.map((item: UniteLegaleHistoryItem) => {
                                    const isFocused = currentHistoryItem ? currentHistoryItem.rid === item.rid : false

                                    return (
                                        <Link key={item.name} to={item.link} className={`${isFocused ? 'bg-slate-100' : 'hover:bg-slate-100 text-slate-700'} inline-flex items-center gap-2 group px-4 py-2`}>
